...
def tanh_op_grad(x,y,y):
    # prepare inputs and outputs
    ins = {'X' : x,'Y' : y,'Y@GRAD' : y}
    attrs = {}
    outs = {}
    out_names = ['X@GRAD']
    ...
    if in_dygraph_mode():
        ...
        core.eager._run_custom_op(ctx, "tanh_op_grad", True)
    else:
        if _in_legacy_dygraph():
            for out_name in out_names:
                outs[out_name] = VarBase()
            _dygraph_tracer().trace_op(type="tanh_op_grad", inputs=ins, outputs=outs, attrs=attrs)
        else:
            helper = LayerHelper("tanh_op_grad", **locals())
            for out_name in out_names:
                outs[out_name] = helper.create_variable(dtype='float32')

            helper.append_op(type="tanh_op_grad", inputs=ins, outputs=outs, attrs=attrs)
